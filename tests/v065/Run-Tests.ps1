param([string[]]$Names=@('menu-layer-tests'))
...
$classes=@{'menu-layer-tests'='MenuLayerTests'}
foreach($name in $Names){
 ...
 if($name -match 'menu|ui|music-position'){Copy-Item -LiteralPath (Join-Path $bundle 'assets') -Destination $runtime -Recurse}
