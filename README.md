...
.\build-release.ps1
.\dist\Kiana-Desktop-Pet-0.8.0-Windows\install.ps1 -InstallRoot 'E:\Kiana-Desktop-Pet\runtime\KianaDesktopPet'
```
...
三套 Q 版新增耳机倾听、轻柔挥棒和活力应援，共 18 张绘制关键姿态。**设置 → 音乐 → 听歌互动与副歌应援** 选择整体模式，默认安静听歌；抒情歌不会因进入副歌就突然打 call。普通三套保持原动作。

**音乐栏右键 → 这首歌的互动方式** 可记住本曲模式，也能标记副歌起点、终点。连续播放约 8 秒后开始倾听，进入已知副歌才按所选模式举棒；无数据就保持耳机。暂停超过 3 秒、拖动、任务活动、睡眠和免打扰会中断或让出动作，应援时保留耳机。

副歌接口已实测，但属于非官方接入，部分歌曲没有数据或只提供一段。自动查询可关闭，手动标记仍可用；仅查询歌曲 ID，不录音、不分析音量、不获取账户凭证。结果会缓存，本曲设置随通用偏好迁移，设置页面底部提供独立动作预览。

![每行从左到右：原待机、耳机、轻柔、活力](docs/demo/music-actions.png)

源码仓库中另有 [音乐动画、数据规则及生成记录](docs/music-animations.md)。
