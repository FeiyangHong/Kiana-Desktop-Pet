param([string[]]$Names=@('docking-tests'))
...
$classes=@{'docking-tests'='DockingTests'}
foreach($name in $Names){
 ...
 if($name -match 'docking|ui|music-position'){Copy-Item -LiteralPath (Join-Path $bundle 'assets') -Destination $runtime -Recurse}
