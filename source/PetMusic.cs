...
namespace KianaPet {
 public sealed partial class PetWindow {
  ...
  async void PollMusic(object sender,EventArgs e){if(musicPolling||quitting)return;musicPolling=true;try{await Music.Poll(Settings.MusicEnabled);if(quitting)return;musicLibrary.Update(Music.Current,Settings.MusicEnabled&&Settings.MusicLyrics);UpdateMusic();}finally{musicPolling=false;if(!quitting){musicFailures=Music.Current.Connected?0:musicFailures+1;double normal=!Settings.MusicEnabled||sessionLocked||!IsVisible?4:.85;musicTimer.Interval=TimeSpan.FromSeconds(Settings.ResourceSaving?PolishRules.RetrySeconds(musicFailures,normal):.85);}}}
