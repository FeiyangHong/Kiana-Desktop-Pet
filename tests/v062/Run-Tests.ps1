param([string[]]$Names=@('gradient-tests'))
...
$classes=@{'gradient-tests'='GradientTests'}
foreach($name in $Names){
 ...
 if($name -match 'gradient|ui|music-position'){Copy-Item -LiteralPath (Join-Path $bundle 'assets') -Destination $runtime -Recurse}
