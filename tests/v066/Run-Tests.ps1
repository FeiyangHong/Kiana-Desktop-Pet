param([string[]]$Names=@('layer-recovery-tests'))
...
$classes=@{'layer-recovery-tests'='LayerRecoveryTests'}
foreach($name in $Names){
 ...
 if($name -match 'layer|ui|music-position'){Copy-Item -LiteralPath (Join-Path $bundle 'assets') -Destination $runtime -Recurse}
